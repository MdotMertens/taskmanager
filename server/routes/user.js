...
module.exports = (userRepository, teamRepository) => {
	...
	router.post('/register', async (req, res) => {
		const data = req.body
		const validData = checkUser.checkRegisterJSON(data) //validate data against the excepted schema
	...
	router.post('/login', async (req, res) => {
		const data = req.body
		const validData = checkUser.checkLoginJSON(data)
		if (validData) {
		...
		} else res.status(401).json({ status: 'Error', message: checkUser.checkLoginJSON.errors })
	})
